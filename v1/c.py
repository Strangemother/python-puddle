...
def name_main():
    ...
    primary = pooler.Async(proc_server, proc_socket_handler, count=4)
    secondaries = pooler.Sync(primary_proc_main, count=20, offset=len(primary))
    return pooler.submit(primary, secondaries, count=-1)
...
async def a_primary_proc_main(*a, **kw):
    ...
    try:
        await extras.async_sleep_tick(*a, **kw)
    except KeyboardInterrupt:
        print('KB Kill',a)
